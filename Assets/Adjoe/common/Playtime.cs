...
namespace io.adjoe.sdk
{
    ...
    public class Playtime : MonoBehaviour
    {
        ...
        // Delegates and static fields for ShowCatalogWithOptions
        private delegate void ShowCatalogWithOptionsOnSuccessDelegate();
        private delegate void ShowCatalogWithOptionsOnErrorDelegate(IntPtr errorPtr);

        private static Action showCatalogWithOptionsSuccessCallback;
        private static Action<Exception> showCatalogWithOptionsErrorCallback;
        private static IntPtr showCatalogWithOptionsOptionsPtr = IntPtr.Zero;

        // Delegates and static fields for SetPlaytimeOptions
        private delegate void SetPlaytimeOptionsOnSuccessDelegate();
        private delegate void SetPlaytimeOptionsOnErrorDelegate(IntPtr errorPtr);

        private static Action setPlaytimeOptionsSuccessCallback;
        private static Action<Exception> setPlaytimeOptionsErrorCallback;
        private static IntPtr setPlaytimeOptionsOptionsPtr = IntPtr.Zero;

        [AOT.MonoPInvokeCallback(typeof(ShowCatalogWithOptionsOnSuccessDelegate))]
        private static void OnShowCatalogWithOptionsSuccess()
        {
            if (showCatalogWithOptionsSuccessCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(showCatalogWithOptionsSuccessCallback);
            }

            // Free allocated memory after callback
            if (showCatalogWithOptionsOptionsPtr != IntPtr.Zero)
            {
                Marshal.FreeHGlobal(showCatalogWithOptionsOptionsPtr);
                showCatalogWithOptionsOptionsPtr = IntPtr.Zero;
            }
        }

        [AOT.MonoPInvokeCallback(typeof(ShowCatalogWithOptionsOnErrorDelegate))]
        private static void OnShowCatalogWithOptionsError(IntPtr errorPtr)
        {
            var errorMessage = Marshal.PtrToStringAnsi(errorPtr);
            if (showCatalogWithOptionsErrorCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(() => showCatalogWithOptionsErrorCallback(new Exception(errorMessage)));
            }
        ...
        [AOT.MonoPInvokeCallback(typeof(SetPlaytimeOptionsOnSuccessDelegate))]
        private static void OnSetPlaytimeOptionsSuccess()
        {
            if (setPlaytimeOptionsSuccessCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(setPlaytimeOptionsSuccessCallback);
            }

            // Free allocated memory after callback
            if (setPlaytimeOptionsOptionsPtr != IntPtr.Zero)
            {
                Marshal.FreeHGlobal(setPlaytimeOptionsOptionsPtr);
                setPlaytimeOptionsOptionsPtr = IntPtr.Zero;
            }
        }

        [AOT.MonoPInvokeCallback(typeof(SetPlaytimeOptionsOnErrorDelegate))]
        private static void OnSetPlaytimeOptionsError(IntPtr errorPtr)
        {
            var errorMessage = Marshal.PtrToStringAnsi(errorPtr);
            if (setPlaytimeOptionsErrorCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(() => setPlaytimeOptionsErrorCallback(new Exception(errorMessage)));
            }
        ...
        // Delegates and static fields for GetStatus
        private delegate void GetStatusOnSuccessDelegate(IntPtr responseJsonPtr);
        private delegate void GetStatusOnErrorDelegate(IntPtr errorPtr);

        private static Action<PlaytimeStatus> getStatusSuccessCallback;
        private static Action<Exception> getStatusErrorCallback;

        [AOT.MonoPInvokeCallback(typeof(GetStatusOnSuccessDelegate))]
        private static void OnGetStatusSuccess(IntPtr responseJsonPtr)
        {
            var responseJson = Marshal.PtrToStringAnsi(responseJsonPtr);
            var response = GenericJsonConverter.JsonToObject<PlaytimeStatus>(responseJson);

            if (getStatusSuccessCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(() => getStatusSuccessCallback(response));
            }
        }

        [AOT.MonoPInvokeCallback(typeof(GetStatusOnErrorDelegate))]
        private static void OnGetStatusError(IntPtr errorPtr)
        {
            var errorMessage = Marshal.PtrToStringAnsi(errorPtr);
            if (getStatusErrorCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(() => getStatusErrorCallback(new Exception(errorMessage)));
            }
        }

        // Delegates and static fields for GetUserID
        private delegate void GetUserIDOnSuccessDelegate(IntPtr userIdPtr);
        private delegate void GetUserIDOnErrorDelegate(IntPtr errorPtr);

        private static Action<string> getUserIDSuccessCallback;
        private static Action<Exception> getUserIDErrorCallback;

        [AOT.MonoPInvokeCallback(typeof(GetUserIDOnSuccessDelegate))]
        private static void OnGetUserIDSuccess(IntPtr userIdPtr)
        {
            var userId = userIdPtr != IntPtr.Zero ? Marshal.PtrToStringAnsi(userIdPtr) : null;

            if (getUserIDSuccessCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(() => getUserIDSuccessCallback(userId));
            }
        }

        [AOT.MonoPInvokeCallback(typeof(GetUserIDOnErrorDelegate))]
        private static void OnGetUserIDError(IntPtr errorPtr)
        {
            var errorMessage = Marshal.PtrToStringAnsi(errorPtr);
            if (getUserIDErrorCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(() => getUserIDErrorCallback(new Exception(errorMessage)));
            }
        }

        // Delegates and static fields for Teardown
        private delegate void TeardownOnSuccessDelegate();
        private delegate void TeardownOnErrorDelegate(IntPtr errorPtr);

        private static Action teardownSuccessCallback;
        private static Action<Exception> teardownErrorCallback;

        [AOT.MonoPInvokeCallback(typeof(TeardownOnSuccessDelegate))]
        private static void OnTeardownSuccess()
            {
                if (teardownSuccessCallback != null)
                {
                    // Ensure callback runs on main thread
                    Dispatcher.RunOnMainThread(teardownSuccessCallback);
                }
            }

        [AOT.MonoPInvokeCallback(typeof(TeardownOnErrorDelegate))]
        private static void OnTeardownError(IntPtr errorPtr)
        {
            var errorMessage = Marshal.PtrToStringAnsi(errorPtr);
            if (teardownErrorCallback != null)
            {
                // Ensure callback runs on main thread
                Dispatcher.RunOnMainThread(() => teardownErrorCallback(new Exception(errorMessage)));
            }
        }
        ...
        /// <summary>
        /// Show the Playtime catalog with the specified options.
        /// </summary>
        /// <param name="options">The options for showing the catalog.</param>
        ...
        public static void ShowCatalogWithOptions(PlaytimeOptions options, Action onSuccess, Action<Exception> onError)
        {
            #if UNITY_IOS
            var optionsJson = GenericJsonConverter.ObjectToJson(options);
            showCatalogWithOptionsOptionsPtr = Marshal.StringToHGlobalAnsi(optionsJson);

            showCatalogWithOptionsSuccessCallback = onSuccess;
            showCatalogWithOptionsErrorCallback = onError;

            PlaytimeiOS.Playtime_showCatalogWithOptions(showCatalogWithOptionsOptionsPtr, OnShowCatalogWithOptionsSuccess, OnShowCatalogWithOptionsError);
            ...
            try {
                playtime.CallStatic("showCatalog", parameters);
                onSuccess();
            } catch (Exception err) {
                onError(err);
            }
            ...
            #elif UNITY_EDITOR
            Debug.Log("Called Playtime.ShowCatalogWithOptions(options=" + options + ", onSuccess=" + onSuccess + ", onError=" + onError + ")");
            #endif
        }

        /// <summary>
        /// Sets the Playtime options globally for the SDK.
        /// </summary>
        /// <param name="options">The options to set.</param>
        /// <param name="onSuccess">Callback called when the operation succeeds.</param>
        /// <param name="onError">Callback called when the operation fails.</param>
        public static void SetPlaytimeOptions(PlaytimeOptions options, Action onSuccess, Action<Exception> onError)
        {
            #if UNITY_IOS
            var optionsJson = GenericJsonConverter.ObjectToJson(options);
            setPlaytimeOptionsOptionsPtr = Marshal.StringToHGlobalAnsi(optionsJson);

            setPlaytimeOptionsSuccessCallback = onSuccess;
            setPlaytimeOptionsErrorCallback = onError;

            // Use static callbacks instead of lambdas
            PlaytimeiOS.Playtime_setPlaytimeOptions(setPlaytimeOptionsOptionsPtr, OnSetPlaytimeOptionsSuccess, OnSetPlaytimeOptionsError);
            ...
            object[] parameters = {
                javaOptions,
                new PlaytimeOptionsListener(onSuccess, onError)
            };
            playtime.CallStatic("setPlaytimeOptions", parameters);

            #elif UNITY_EDITOR
            Debug.Log("Called Playtime.SetPlaytimeOptions(options=" + options + ", onSuccess=" + onSuccess + ", onError=" + onError + ")");
            #endif
        }
        ...
        /// <param name="onSuccess">Callback called when the operation succeeds.</param>
        /// <param name="onError">Callback called when the operation fails.</param>
        public static void GetStatus(Action<PlaytimeStatus> onSuccess, Action<Exception> onError)
        {
            #if UNITY_IOS
            getStatusSuccessCallback = onSuccess;
            getStatusErrorCallback = onError;

            // Use static callbacks instead of lambdas
            PlaytimeiOS.Playtime_getStatus(OnGetStatusSuccess, OnGetStatusError);
            #elif UNITY_ANDROID
            ...
            object[] parameters = {};
            AndroidJavaObject statusJava = playtime.CallStatic<AndroidJavaObject>("getStatus", parameters);
            PlaytimeStatus status = new PlaytimeStatus(statusJava);
            ...
            #elif UNITY_EDITOR
            Debug.Log("Called Playtime.GetStatus(onSuccess=" + onSuccess + ", onError=" + onError + ")");
            #endif
        }

        /// <summary>
        /// Fetches the user ID from the Playtime SDK.
        /// </summary>
        /// <param name="onSuccess">Callback called when the operation succeeds with the user ID.</param>
        /// <param name="onError">Callback called when the operation fails.</param>
        public static void GetUserID(Action<string> onSuccess, Action<Exception> onError)
        {
            #if UNITY_IOS
            getUserIDSuccessCallback = onSuccess;
            getUserIDErrorCallback = onError;

            // Use static callbacks instead of lambdas
            PlaytimeiOS.Playtime_getUserId(OnGetUserIDSuccess, OnGetUserIDError);
            #elif UNITY_ANDROID
        ...
        /// <param name="onSuccess">Callback called when the operation succeeds.</param>
        /// <param name="onError">Callback called when the operation fails.</param>
        public static void Teardown(Action onSuccess, Action<Exception> onError)
        {
            #if UNITY_IOS
            teardownSuccessCallback = onSuccess;
            teardownErrorCallback = onError;

            PlaytimeiOS.Playtime_teardown(OnTeardownSuccess, OnTeardownError);
            #elif UNITY_ANDROID
            try
            {
                playtime.CallStatic("teardown", GetCurrentContext());
                onSuccess();
            }
            catch (Exception ex)
            {
        ...
        internal static AndroidJavaObject GetJavaPlaytimeParams(PlaytimeParams playtimeParams)
        {
            AndroidJavaObject builder = new AndroidJavaObject("io.adjoe.sdk.PlaytimeParams$Builder");
                if (playtimeParams != null)
                {
                    ...
                    builder.Call<AndroidJavaObject>("setPromotionTag", new object[] {playtimeParams.promotionTag});
                }
        ...
        private class PlaytimeOptionsListener : AndroidJavaProxy
        {
            private readonly Action successCallback;
            private readonly Action<Exception> errorCallback;

            public PlaytimeOptionsListener(Action successCallback, Action<Exception> errorCallback)
                    : base("io.adjoe.sdk.PlaytimeOptionsListener")
            {
                this.successCallback = successCallback;
                this.errorCallback = errorCallback;
            }
            ...
            public void onError(string error)
            {
                ...
                Exception optionsError = new Exception(error);

                if (useLegacyCallbacks)
                {
                    errorCallback(optionsError);
                }
                else
                {
                    Dispatcher.RunOnMainThread(() => {
